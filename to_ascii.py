...
Functions:
    ...
    img_to_ascii(Image object/3D Numpy array, int/float) -> (tuple, map object).
        Convert Image object or RGB pixel Numpy array into map object
        of 2D ASCII character array.
...
def brightnesses_to_asciis(brightness_list):
    ...
    Returns:
        Map object. 1D array of the corresponding ASCII characters for
        each brightness.
    """
...
def img_to_ascii(img):
    """Convert Image object or RGB pixel Numpy array into map object of
    2D ASCII character array.

    Parameters:
        img: PIL Image object or RGB pixel Numpy array.
    Returns:
        Map object. 2D array of ASCII characters representing each
    ...
    if type(img) != np.ndarray:
        img = np.array(img)
    if np.ndim(img) != 3 or len(img[0][0]) != 3:
        raise ValueError('Expect data as 3D array of RGB data.')
...
def text_image(ascii_characters, image_shape):
    """Draw 2D array of ASCII characters onto PIL Image.

    Parameters:
        ascii_characters: 2D array. ASCII characters for drawing
            onto image.
            [
                [row1],
                [row2],
    ...
    default_font = ImageDraw.Draw(Image.new('RGB', (1, 1), (0, 0, 0))).getfont()
